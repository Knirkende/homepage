...
def _wiki_lookup(name, *args, **kwargs):
	...
	lookup_name = name.replace(
						...
						',', '').replace(' ', '_')
	lookup_name = re.sub('(?<=\w)_$', '', lookup_name)
	#Some manual cleanup of urls.
	if lookup_name == 'Catilina':
		lookup_name = 'Catilina_(drama)'
	elif lookup_name == 'Svanhild':
		lookup_name = 'Svanhild_(komedie)'
	elif lookup_name == 'Kongs-emnerne':
		lookup_name = 'Kongs-Emnerne'
	elif lookup_name == 'De_Unges_Forbund':
		lookup_name = lookup_name.replace('U', 'u')
	...
	try:
		...
		return (f'Fant ingen artikkel om {name} på norsk wikipedia.', URL)
	return (parag, URL)

def word_picker(*args, **kwargs):
	"""
	Returns a tuple containing (key, value, info_paragraph, url) where key,
	then value are randomly picked from the specified json file and
	info_paragraph and source URL is served by helper function _wiki_lookup.
	"""
	...
	info_paragraph, url = _wiki_lookup(name = choice_key)
	return (choice_key, choice_value, info_paragraph, url)